...
/**
 * Base class for integration tests using MySQL 8.0 with {@code caching_sha2_password} authentication.
 * <p>
 ...
 * configured with:
 * <ul>
 *   <li>MySQL 8.0.35 (default, configurable via {@code test.mysql.sha2.version} system property)</li>
 *   <li>{@code caching_sha2_password} as the default authentication plugin (MySQL 8.0 default)</li>
 ...
 *
 *     &#64;Test
 *     void testCachingSha2Authentication() throws SQLException {
 *         // Create a test user with caching_sha2_password authentication
 ...
 * <ul>
 *   <li>{@link #createUser(String, String)} - Create MySQL user with caching_sha2_password auth</li>
 ...
 * {@code caching_sha2_password} is the default authentication plugin for MySQL 8.0+. It provides:
 * <ul>
 *   <li>More secure password hashing than {@code mysql_native_password}</li>
 *   <li>Server-side password caching for performance</li>
 *   <li>RSA public key encryption for initial password exchange</li>
 *   <li>Requires secure connection or RSA encryption for first authentication</li>
 * </ul>
 * <p>
 * <b>Configuration:</b>
 * The MySQL version can be customized via system property:
 * <pre>
 * -Dtest.mysql.sha2.version=8.0.36
 * </pre>
 ...
 * @see AbstractSha256PasswordTest
 */
public abstract class AbstractCachingSha2PasswordTest extends AbstractAuthenticationTest {

    /**
     * Default MySQL version for caching_sha2_password authentication tests.
     * Can be overridden via {@code test.mysql.sha2.version} system property.
     */
    ...
    private static final String MYSQL_VERSION = SystemPropertyUtil.get(
        "test.mysql.sha2.version", DEFAULT_MYSQL_VERSION);
        ...
        try {
            ...
            Runtime.getRuntime().addShutdownHook(new Thread(() -> {
                try {
                    ...
                    System.err.println("Failed to close JDBC datasource for MySQL caching_sha2_password tests: " +
                                     e.getMessage());
                } finally {
                    try {
                        ...
                        System.err.println("Failed to stop MySQL container for caching_sha2_password tests: " +
                                         e.getMessage());
                    }
                }
            }, "mysql-caching-sha2-test-cleanup"));
            ...
            throw new IllegalStateException(
                "Failed to initialize MySQL container for caching_sha2_password authentication tests. " +
    ...
    /**
     * Creates the MySQL container with caching_sha2_password authentication configuration.
     * <p>
     * The container is configured with:
     * <ul>
     *   <li>Version specified by {@code MYSQL_VERSION}</li>
     *   <li>{@code caching_sha2_password} as default auth plugin</li>
     ...
     *
     * @return the MySQL 8.0 container with caching_sha2_password authentication
     */
    @Override
    ...
    /**
     * Creates a MySQL user with {@code caching_sha2_password} authentication.
     * <p>
     ...
     * <p>
     * <b>Note:</b> First authentication with caching_sha2_password requires either
     * a secure connection or RSA key exchange. The R2DBC driver handles this automatically.
     *
