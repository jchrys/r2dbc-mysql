...
/**
 * Base class for integration tests using MySQL 5.7 with {@code mysql_native_password} authentication.
 * <p>
 * This class provides a shared MySQL 5.7 container instance that is started once during class
 * initialization and reused across all test classes that extend this base. The container is
 * configured with:
 * <ul>
 *   <li>MySQL 5.7.44 (default, configurable via {@code test.mysql.native.version} system property)</li>
 *   <li>{@code mysql_native_password} as the default authentication plugin</li>
 ...
 * <b>Usage Example:</b>
 * <pre class="code">
 * class MyAuthenticationTest extends AbstractMySqlNativePasswordTest {
 *
 *     &#64;Test
 *     void testNativePasswordAuthentication() throws SQLException {
 ...
 * <ul>
 *   <li>{@link #createUser(String, String)} - Create MySQL user with native password auth</li>
 ...
 * <pre>
 * -Dtest.mysql.native.version=5.7.42
 * </pre>
...
public abstract class AbstractMySqlNativePasswordTest extends AbstractAuthenticationTest {
    ...
    /**
     * Default MySQL version for native password authentication tests.
     * Can be overridden via {@code test.mysql.native.version} system property.
     */
    private static final String DEFAULT_MYSQL_VERSION = "5.7.44";
    ...
    private static final String MYSQL_VERSION = SystemPropertyUtil.get(
        "test.mysql.native.version", DEFAULT_MYSQL_VERSION);
        ...
        try {
            ...
            Runtime.getRuntime().addShutdownHook(new Thread(() -> {
                try {
                    ...
                    System.err.println("Failed to close JDBC datasource for MySQL native password tests: " +
                                     e.getMessage());
                } finally {
                    try {
                        ...
                        System.err.println("Failed to stop MySQL container for native password tests: " +
                                         e.getMessage());
                    }
                }
            }, "mysql-native-password-test-cleanup"));
            ...
            throw new IllegalStateException(
                "Failed to initialize MySQL container for native password authentication tests. " +
    ...
    /**
     * Creates the MySQL container with native password authentication configuration.
     * <p>
     * The container is configured with:
     * <ul>
     *   <li>Version specified by {@code MYSQL_VERSION}</li>
     *   <li>{@code mysql_native_password} as default auth plugin</li>
     ...
     *
     * @return the MySQL 5.7 container with native password authentication
     */
    @Override
    ...
    /**
     * Creates a MySQL user with {@code mysql_native_password} authentication.
     * <p>
